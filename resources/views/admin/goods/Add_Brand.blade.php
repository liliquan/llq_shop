        ...
        <div id="add_brand" class="clearfix">
            <div class="left_add">
                    ...
                    <div class="title_name">添加品牌</div>
                        <form action="{{ route('add_handle') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                            <ul class="add_conent">
                                <li class=" clearfix"><label class="label_name"><i>*</i>品牌名称：</label> <input name="brand_name" type="text"
                                        class="add_text" /></li>
                                ...
                                <li class=" clearfix"><label class="label_name"><i>*</i>所属地区：</label> <input name="country" type="text"
                                        class="add_text" style="width:120px" /></li>
                                <li class=" clearfix"><label class="label_name">品牌描述：</label> <textarea name="desc" cols="" rows=""
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        seach.keyup(function (event) {
            ...
            var product =
                "<div class='title_name'>产品名称</div><select multiple='multiple' id='select1' class='select'>";
            if (seachText != "") {
                $.each(user, function (id, item) {
                    //如果包含则为select赋值
                    if (item.name.indexOf(seachText) != -1 && item.status.indexOf(seachText) !=
                        -1) {
                        product += "<option value=" + item.id + ">" + "(" + item.status + ")" +
                            item.name + "</option>";
                    }
                })
        ...
        $.each(user, function (id, item) {
            name = item.name;
            status = item.status;
            product += "<option value=" + item.id + " title=" + item.name + ">" + "(" + item.status +
                ")" + item.name + "</option>";
